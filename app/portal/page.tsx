...
export default function PortalPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [appsRes, deptRes, favRes] = await Promise.all([
        fetch('/api/apps'),
        fetch('/api/admin/departments'),
        fetch('/api/favourites'),   // ← DB-backed favourites
      ]);

      if (appsRes.ok) { const { apps } = await appsRes.json(); setApps(apps); }
      if (deptRes.ok) { const { depts } = await deptRes.json(); setDeptTree(depts); }
      if (favRes.ok)  { const { favourites } = await favRes.json(); setFavorites(favourites); }
  ...
  // ── Toggle: optimistic UI + DB write ──
  const toggleFav = (id: number) => {
    setFavorites(prev => prev.includes(id) ? prev.filter(f => f !== id) : [...prev, id]);
    // API call is fired inside AppCard (handleFav), so no duplicate call needed here
  };
  ...
  return (
    <div className="min-h-screen grid-bg">
      ...
      <main className="max-w-screen-2xl mx-auto px-4 sm:px-6 pb-16">
            ...
            {statCards.map((card, i) => {
              ...
              return (
                <motion.button key={card.type}
                  ...
                  style={{ background: isActive ? card.activeBg : 'var(--surface)', border: `1px solid ${isActive ? card.activeBorder : 'var(--border)'}`, cursor: 'pointer' }}
                >
                  <div className="w-8 h-8 rounded-xl flex items-center justify-center mb-3" style={{ background: card.activeBg }}>
        ...
        {/* ── Favourites section (DB-backed, shown at top) ── */}
        {favApps.length > 0 && (
